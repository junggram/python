...
print('공분산 ------------')
print(np.cov(df['친밀도'],df['적절성'])) # np.cov는 두개씩만 볼 수 있다
print(np.cov(df['친밀도'],df['만족도']))
print()
print(df.cov()) # pandas.cov는 한번에 다 볼 수 있다

print('상관계수 ------------')
print(np.corrcoef(df['친밀도'],df['적절성']))
print(np.corrcoef(df['친밀도'],df['만족도']))
print()
print(df.corr()) # prandas.cor 
print(df.corr(method='pearson')) # 기본값이 피어슨 # 등간, 비율 척도, 정규성을 따름

print(df.corr(method='spearman'))  # 서열 척도, 정규성을 따르지않음
print(df.corr(method='kendall'))  # 스피어만과 유사함

print()
# 만족도에 대한 다른 특성 사이의 상관관계 출력 (하나의 특성에 대한 다른특성들의 상관관계 출력)
co_re = df.corr()
