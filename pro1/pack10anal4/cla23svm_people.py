...
faces = fetch_lfw_people(min_faces_per_person = 60, color=False) # 최소 60명, 흑백사진
print(faces)
print(faces.DESCR)
...
# 주성분 분석으로 이미지 차원 축소
m_pca = PCA(n_components = 150, whiten=True, random_state=0) # whiten : 주성분의 스케일이 작아지도록 조정
...
m_svc = SVC(C=1)
model = make_pipeline(m_pca, m_svc) # 선처리기(주성분 분석)와 분류기를()하나의 pipeline 으로 묶어 순차적으로 진행시킴 
print(model)
...
print('con_mat :\n', con_mat)
print('분류정확도 :', accuracy_score(y_test, pred)) # 분류정확도 : 0.795

fig, ax = plt.subplots(4,6)
for i, axi in enumerate(ax.flat):
    axi.imshow(x_test[i].reshape(62,47), cmap='bone')
    axi.set(xticks=[], yticks=[])
    axi.set_ylabel(faces.target_names[pred[i]].split()[-1], color = 'blue' if pred[i] == y_test[i] else 'red')
