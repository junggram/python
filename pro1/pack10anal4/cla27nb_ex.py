...
plt.show() # spore-print-color : 330, odor : 139, gill-size : 88, cap-color : 58  # 상위4개만 뽑음
...
print('총갯수 :%d, 오류수:%d'%(len(y_test), (y_test != pred).sum())) # 총갯수 :2438, 오류수:723
print('분류정확도 :', metrics.accuracy_score(y_test, pred)) # 분류정확도 : 0.7034454470877769
...
fig, ax = plt.subplots(figsize = (10, 12))
plot_importance(model, ax = ax) 
plt.show() # spore-print-color : 330, odor : 139, gill-size : 88, cap-color : 58, population : 45  # 상위4개만 뽑음

i_features = data[['spore-print-color', 'odor', 'gill-size', 'cap-color', 'population']] # 중요변수 따로 뽑아서 담아줌
x_train, x_test, y_train, y_test = train_test_split(i_features, labels, test_size = 0.3, random_state=1)
...
print('예상치 :', pred[:3])
print('실제값 :', y_test[:3].values)
print('총갯수 :%d, 오류수:%d'%(len(y_test), (y_test != pred).sum())) # 총갯수 :2438, 오류수:656
print('분류정확도 :', metrics.accuracy_score(y_test, pred)) # 분류정확도 : 0.7309269893355209
'''